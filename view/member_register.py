...
table_members = db.Table('Member', metadata, autoload_with=engine)
table_occupation = db.Table('Occupation', metadata, autoload_with=engine)
...
def member_info_show():
    ...
    sq = db.select(table_members).limit(
        each_page).offset((page-1)*each_page).subquery()
    query = db.select(sq, table_occupation.c.Occupation).select_from(
        sq.outerjoin(table_occupation))
    proxy = connection.execute(query)
    sql_results = proxy.fetchall()
    tmp_df = pd.DataFrame(sql_results, columns=[
...
def member_edit_info():
    ...
    if request.method == "POST":
        try:
            connection = engine.connect()  # connection 要放在view function中，否則會出現thread error
            query = db.select(table_members.c.Member_ID).order_by(
                table_members.c.Member_ID)
            proxy = connection.execute(query)
            id_list = [idx[0] for idx in proxy.fetchall()]
            # 希望至少要填寫名子 或是 刪除會員
            if request.form['Name'] or ('Delete_member' in request.form):
                if 'Delete_member' in request.form:
                    query = db.delete(table_members).where(
                        table_members.c.Member_ID == request.form['Member_ID'])
                    connection.execute(query)
                else:
                    query = db.update(table_members).where(table_members.c.Member_ID == request.form['Member_ID']).values(
                        **{k: request.form[k] for k in ['Name', 'Unit']})
                    connection.execute(query)
                    if "Occupation" in request.form:
                        query = db.delete(table_occupation).where(
                            table_occupation.c.Member_ID == request.form['Member_ID'])
                        connection.execute(query)
                        query = db.insert(table_occupation).values([(request.form['Member_ID'], request.form.getlist(
                            'Occupation')[i]) for i in range(len(request.form.getlist('Occupation')))])
                        connection.execute(query)
                        connection.commit()  # 額外加上（雲
            else:
                raise Exception
    ...
    if request.method == "POST":
        try:
            ...
            if request.form['Name'] and request.form['Member_ID']:  # 希望至少要填寫名子
                query = db.insert(table_members).values(
                    **{k: request.form[k] for k in request.form.keys()})
                proxy = connection.execute(query)
            else:
                raise Exception
        except:
