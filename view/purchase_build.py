...
pc_build_app = Blueprint('purchase_build_app',
                         __name__, url_prefix="/pc_build")
...
table_PurchasingList = db.Table(
    table_name_PurchasingList, metadata, autoload_with=engine)
...
def index():  # show PurchasingList
    ...
    connection.commit()
    query = db.select(table_PurchasingList).limit(
    ...
    if request.method == "POST":
        try:
            ...
            if request.form['List_ID'] and request.form['Item'] and request.form['Specification'] and request.form['Amount']:
                query = db.select(table_PurchasingList.c.Sn).select_from(
                    table_PurchasingList).order_by(table_PurchasingList.c.Sn.desc())
                proxy = connection.execute(query)
                connection.commit()
                ...
                query = db.insert(table_PurchasingList).values(
                    List_ID=request.form['List_ID'], Sn=PurchasingList_SN_GENERATE, Item=request.form['Item'], Specification=request.form['Specification'], Amount=request.form['Amount'])
                proxy = connection.execute(query)
                connection.commit()
            else:
                raise Exception
...
def purchasing_list_delete():
    if request.method == "POST":
        connection = engine.connect()
        query = db.select(table_PurchasingList.c.Sn).order_by(
            table_PurchasingList.c.Sn)
        proxy = connection.execute(query)
        id_list = [idx[0] for idx in proxy.fetchall()]
        try:
            ...
            sql = db.delete(table_PurchasingList).where(
                table_PurchasingList.c.Sn == request.form['Sn'])
